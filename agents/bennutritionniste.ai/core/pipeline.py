...
config_path = Path(__file__).parent.parent / "config" / "instagram_config.json"
with open(config_path, 'r', encoding='utf-8') as f:
    config = json.load(f)

# Extract config values
INSTAGRAM_USER = config["instagram"]["user"]
INSTAGRAM_PASS = config["instagram"]["password"]
TARGET_ACCOUNT = config["instagram"]["target_account"]
OPENAI_API_KEY = os.getenv(config["openai"]["api_key_env"], config["openai"]["api_key_fallback"])
VIDEO_DIR = config["directories"]["video_dir"]
TRANSCRIPTS_DIR = config["directories"]["transcripts_dir"]
CHROMA_DB_DIR = config["directories"]["chroma_db_dir"]
