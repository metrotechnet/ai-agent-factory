    ...
    languageSelector: {
        render(config) {
            ...
            if (type === 'pair') {
                ...
                if (sourceSelect && config.languages) {
                    this.populateLanguages(sourceSelect, config.languages, config.defaultSource || config.source?.default);
                }
                
                // Populate target language options
                if (targetSelect && config.languages) {
                    this.populateLanguages(targetSelect, config.languages, config.defaultTarget || config.target?.default);
                }
            } else {
        ...
        populateLanguages(select, languages, defaultValue) {
            ...
            for (const [code, label] of Object.entries(languages)) {
                ...
                // Set selected attribute if this is the default
            ...
            // Also set select.value as a fallback
