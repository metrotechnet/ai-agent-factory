...
1. **Copy existing prompts**:
   ```powershell
   Copy-Item knowledge-base\nutria\prompts.json knowledge-base\my-new-kb\prompts.json
   ```
...
Remove-Item knowledge-base\my-kb-name\chroma_db -Recurse -Force
...
2. Verify documents exist in the folder:
   ```powershell
   Get-ChildItem knowledge-base\my-kb-name\extracted_texts
   ```
